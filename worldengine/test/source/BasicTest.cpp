...
TEST(BasicTest, AntiAlias1x1Test)
{
   boost::multi_array<float, 2> map(boost::extents[1][1]);
   map[0][0] = 0.8f;

   AntiAlias(map, 10);

   EXPECT_FLOAT_EQ(map[0][0], 0.8f);
}

TEST(BasicTest, AntiAlias3x3Test)
{
...
TEST(BasicTest, AntiAlias5x3Test)
{
   boost::multi_array<float, 2> map(boost::extents[3][5]);
   map[0][0] = 0.5f;
   map[0][1] = 0.12f;
   map[0][2] = 0.7f;
   map[0][3] = 0.15f;
   ...
   EXPECT_FLOAT_EQ(map[0][0], 1.2781818f);
   EXPECT_FLOAT_EQ(map[1][2], 0.49181818f);
}
